...
public class SubCamera extends Activity implements OnClickListener {
	...
	class SubCamera1 implements PictureCallback {
		public void onPictureTaken(byte[] paramArrayOfByte, Camera paramCamera) {
		}
	}

	class SubCamera2 implements Callback {
	...
	private void startPreview() {
		if (null != mCamera) {
			...
			if (optimalSize != null) {
				// mofidy at 20130722 by zhu
				mCameraParam.setPreviewSize(optimalSize.height, optimalSize.width);
				// mCameraParam.setPreviewSize(optimalSize.width, optimalSize.height);
				// mofidy at 20130722 by zhu end
			}

			mCamera.setDisplayOrientation(0);// 180
			// end
