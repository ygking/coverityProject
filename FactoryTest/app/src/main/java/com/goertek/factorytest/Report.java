...
public class Report extends Activity
{
  ...
  protected void ShowInfo()
  {
   ...
   // Log.d("yxyx_report","succ="+localStringBuilder.toString());
    StringBuilder localStringBuilder3 = new StringBuilder().append(str2).append("测试失败项:").append(str2);
    int l = this.mFailedList.size();
    for (int k = 0; k < l; k++)
    {
      localStringBuilder3.append((String)this.mFailedList.get(k)).append(str1);
    }
   // Log.d("yxyx_report","fail="+localStringBuilder3.toString());
    this.mFailed.setText(localStringBuilder3.toString());
    StringBuilder localStringBuilder5 = new StringBuilder().append(str2).append("未测试项:").append(str2);
  ...
  protected void onCreate(Bundle paramBundle)
  {
    ...
    for(int i = 0; i < this.itemString.length; i++){
        ...
        if(this.mSp.getString(getString(this.itemString[i]), "not test").equals("success"))
        {
            this.mOkList.add(getString(this.itemString[i]));
        }
        else if(this.mSp.getString(getString(this.itemString[i]), "not test").equals("failed"))
        {
